...
{}
	...
	// TODO: REMOVE private
	//private:
	std::array<SizeType, height> _cellsInRow{0};
	std::array<CellState, height*width> _landedBlocks{0};
	Falling _falling{0, (width-1)/2, SHAPE_L};

	void delete_row(const SizeType row)
	{
		std::move(_landedBlocks.begin(), _landedBlocks.begin() + row*width, _landedBlocks.begin() + width);
		std::move(_cellsInRow.begin(), _cellsInRow.begin() + row, _cellsInRow.begin() + 1);
		std::fill(_landedBlocks.begin(), _landedBlocks.begin() + width, 0);
		_cellsInRow[0] = 0;
		return;
	}

	void set_cell_state(const SizeType i, const SizeType j, const CellState cellState)
	{
	...
	bool falling_has_valid_position() const
	{
		if (_falling.get_upper_left_h() < 0 || _falling.get_upper_left_w() < 0
				|| _falling.get_lower_right_h() >= height || _falling. get_lower_right_w() >= width)
		{
			return false;
		}
		else
		{
			for (SizeType x = _falling.get_upper_left_h(); x <= _falling.get_lower_right_h(); ++x)
			{
				for (SizeType y = _falling.get_upper_left_w(); y <= _falling.get_lower_right_w(); ++y)
				{
					if (get_falling_state(x, y) != CS_DEAD && get_landed_state(x, y) != CS_DEAD)
					{
						return false;
					}
				}
	...
	{
		for (SizeType h = _falling.get_upper_left_h(); h <= _falling.get_lower_right_h(); ++h)
		{
			for (SizeType w = _falling.get_upper_left_w(); w <= _falling.get_lower_right_w(); ++w)
			{
